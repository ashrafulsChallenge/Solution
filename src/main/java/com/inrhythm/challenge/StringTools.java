...
import com.inrhythm.challenge.library.objects.IntStringPair;
import com.inrhythm.challenge.utilities.*;

import java.io.*;

/**
 * Assumptions: 1) Its assumed that only alphanumeric characters are allowed and
 * anything else should be neglected/omitted. If all characters in string are
 * un-allowed then the string should be considered an empty string.
 *
 * 2) Also, it is assumed that for empty strings the method should raise an
 * Exception.
 *
